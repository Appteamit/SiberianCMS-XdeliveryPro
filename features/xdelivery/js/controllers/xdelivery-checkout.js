...
angular
  ...
  .controller(
    ...
    function (
      ...
      $scope.loadContent = function () {
        ...
        Xdelivery.checkoutScreenv2(
          $scope.carts.delivery,
          $scope.carts.main.total_amount,
          $scope.postParams,
          $scope.carts.is_delivery_applied
        )
          .then(
            function (data) {
              $scope.payout = data;
              ...
              $scope.is_today = true;
              $scope.carts.main.total_amount_with_currency =
                data.total_amount_with_currency;

              if (
                data.shipping.is_shipping &&
                $scope.carts.delivery != "pickup"
              ) {
            ...
            function (error) {
              $scope.is_loading = false;
              Dialog.alert(
                $translate.instant("Error", "xdelivery"),
                error.message,
                $translate.instant("OK", "xdelivery"),
                -1
              );
            }
          )
      ...
      $scope.getLocation = function () {
        ...
        if (Location.isEnabled) {
          ...
          Location.getLocation({ timeout: 10000 }, true).then(
            function (position) {
              console.log(
                "Location enabled",
      ...
      $scope.deliveryDateChange = function () {
        ...
        Xdelivery.getSelectTime(
          $scope.carts.delivery,
          $scope.carts.delivery_date,
          $scope.postParams.store_id
        )
          .then(
            function (data) {
      ...
      $scope.continueOrder = function () {
        if ($scope.carts.delivery_date == "" && $scope.is_allow_select_date) {
          Dialog.alert(
            $translate.instant("Error", "xdelivery"),
            $translate.instant("Please select a date", "xdelivery"),
            $translate.instant("OK", "xdelivery"),
            -1
          );
          return true;
        ...
        if ($scope.carts.delivery_time == "" && $scope.is_allow_select_time) {
          Dialog.alert(
            $translate.instant("Error", "xdelivery"),
            $translate.instant("Please select a time", "xdelivery"),
            $translate.instant("OK", "xdelivery"),
            -1
          );
          return true;
        ...
        if (
          $scope.carts.delivery == "delivery" &&
          ($scope.carts.address == "" || $scope.carts.address == null)
        ) {
          Dialog.alert(
            $translate.instant("Error", "xdelivery"),
            $translate.instant("Please select a address", "xdelivery"),
            $translate.instant("OK", "xdelivery"),
            -1
          );
          return true;
  ...
  .controller(
    ...
    function (
      ...
      $scope.loadContent = function () {
        ...
        if (
          Xdelivery.isUndefined($scope.carts.main) ||
          $scope.carts.main.cart_products.length == 0
        ) {
          Loader.show();
          $state
        ...
        Xdelivery.paymentScreen($scope.carts)
          .then(
            function (data) {
              $scope.payout = data;
              $scope.carts.payment_method = "";
      ...
      $scope.updateOrderStatus = function (param) {
        ...
        Loader.show();
        Xdelivery.updatePaymentStatus(param).then(
          function (data) {
            Loader.hide();
            if (param.status == "success") {
              Dialog.alert(
                $translate.instant("Success", "xdelivery"),
                $translate.instant(
            ...
            if (param.status == "failed") {
              Dialog.alert(
                $translate.instant("Error", "xdelivery"),
                $translate.instant("Payment has been failed", "xdelivery"),
                $translate.instant("OK", "xdelivery"),
                -1,
                "xdelivery"
              );
              ...
              .then(function () {
                $state
                  ...
                  .then(function () {
                    $state.go(
                      "xdelivery-orders",
                      { value_id: $scope.value_id, order_id: order_id },
                      { reload: true }
                    );
                  });
              });
      ...
      $scope.placeOrder = function () {
        if ($scope.carts.payment_method == "") {
          Dialog.alert(
            $translate.instant("Error", "xdelivery"),
            $translate.instant("Please select a payment method!", "xdelivery"),
            $translate.instant("OK", "xdelivery"),
            -1
          );
          return true;
        }
        if ($scope.method_type == "nmi") {
          if (!$scope.validateNmiForm()) {
            return false;
            }
            $scope.carts.nmi = $scope.nmi;
        }
        $scope.carts.is_delivery_applied = false;
        Xdelivery.carts = $scope.carts;
          ...
          function (data) {
            ...
            if (data && data.is_paypal && data.data.status == "success") {
              ...
              if (
                data &&
                data.is_stripe &&
                data.stripeData.status == "requires_action"
              ) {
                Xdelivery.StripeInstance.confirmCardPayment(
                  ...
                  if (ccpResponse.error) {
                    ...
                    Dialog.alert(
                      "Error",
                      ccpResponse.error.message,
                      "OK",
                      -1,
                      "payment_stripe"
                    );

                    Xdelivery.authorizationStripeError(
                      data.order_id,
                      data.stripeData.id,
                      ccpResponse.error
                    ).then(
      ...
      $scope.showPopup = function (title, message) {
        Dialog.alert(
          $translate.instant(title, "xdelivery"),
          $translate.instant(message, "xdelivery"),
          $translate.instant("OK", "xdelivery"),
          -1
        );
      };
          ...
          $timeout(function () {
            try {
              ...
              var saveElement = document.getElementById(
                "xdelivery_save_element"
              );
              var displayError = document.getElementById(
                "xdelivery_card_errors"
              );
              var displayErrorParent = document.getElementById(
                "xdelivery_card_errors_parent"
              );
      ...
      $scope.payStripeNow = function () {
        Xdelivery.StripeInstance.createToken($scope.cardElement).then(function (
          result
        ) {
  ...
  .controller(
    "XdeliveryOrderSuccessController",
    function (
      Dialog,
      Loader,
      Customer,
      ...
      $scope.loadContent = function () {
        $scope.is_loading = true;
        Xdelivery.findOrderById($scope.order_id)
          .then(
            function (data) {
              $scope.payout = data;
          ...
          .then(function () {
            ...
            $state
              ...
              .then(function () {
                $state.go(
                  "xdelivery-orders",
                  { value_id: $scope.value_id, order_id: $scope.order_id },
                  { reload: true }
                );
              });
          });
  ...
  .controller(
    "XdeliveryPaypalReturnController",
    function (
      Dialog,
      Xdelivery,
      SB,
