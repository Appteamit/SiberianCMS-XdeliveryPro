...
class Xdelivery_Mobile_OrderController extends Application_Controller_Mobile_Default
{
        ...
        try {
            ...
            if ($value_id = $this->getRequest()->getParam('value_id')) {
                ...
                foreach ($orders as $order) {
                    ...
                    $data['is_return_request'] = (int) $data['is_return_request'];
    ...
    public function processNmiTransaction($securityKey, $amount, $ccNumber, $ccExp, $cvv, $testMode = 1)
    {
        if ($testMode == 1) {
            $testMode = 'enabled';
        } else {
        ...
        // dd($testMode, $securityKey, $amount, $ccNumber, $ccExp, $cvv);
    ...
    public function saveAction()
    {
        try {
            ...
            if ($param = $this->getRequest()->getBodyParams()) {
                ...
                $onlineGateways = ['ewallet', 'paypal', 'nmi'];
                $settingModel = (new Xdelivery_Model_Settings())->find(['value_id' => $value_id]);
                $settings = $settingModel->getData();
                ...
                if (in_array($paymentMethod->getMethodType(), $onlineGateways) || $paymentMethod->getMethodType() == 'stripe') {
                    ...
                    if (!(float)$settings['enable_acceptance_rejection']) {
                ...
                if ($order_id) {
                    ...
                    foreach ($param['main']['cart_products'] as $key => $value) {
                        ...
                        $orderItemModel = (new Xdelivery_Model_OrderItems())
                            ...
                            ->setBasePriceInclTax(((float)$value['price'] + ($value['tax_amount'] / $value['qty'])))
                            ->setChoicePrice($value['sub_options_total'])
                            ->setPrice($value['selling_price'])
                            ->setPriceInclTax(((float)$value['selling_price'] + ($value['tax_amount'] / $value['qty'])))
                            ->setQty($value['qty'])
                    ...
                    // nmi payment
                    if ($paymentMethod->getMethodType() == 'nmi') {
                        // dd($param);
                        $nmiModel = (new Xdelivery_Model_PaymentMethod())->find(['value_id' => $value_id, 'method_type' => 'nmi']);
                        if ($nmiModel->getId()) {
                            $settings_nmi = $nmiModel->getData();
                            $processing_fee = $settings_nmi['processing_fee'];
                            $processing_fee_percent = ($param["main"]['total_amount'] * $settings_nmi['processing_fee']) / 100;
                            $param["main"]['total_amount'] = $param["main"]['total_amount'] + $processing_fee_percent;
                            // dd($param["main"]['total_amount']);
                            // dd($settings_nmi);
                            if (empty($settings_nmi['secret_key'])) {
                                throw new Exception(p__('xdelivery', 'This payment methods are temporarily unavailable'));
                            }
                            $param['nmi']['expiry_date'] = str_replace('-', '/', $param['nmi']['expiry_date']);
                            $response = $this->processNmiTransaction($settings_nmi['secret_key'], $param["main"]['total_amount'], $param["nmi"]['card_number'], $param["nmi"]['expiry_date'], $param["nmi"]['card_cvc'], $settings_nmi['is_test_mode']);
                            // dd($response);
                            if ($response['status'] == 'success') {
                                ...
                                $txnModel = (new Xdelivery_Model_OrderTransactions())
                                    ->find(['order_id' => $order_id])
                                    ->setStatus($payment_status)
                                    ->setGatewayTransactionId($response['transaction_id'])
                                    ->save();
                            } else {
    ...
    public function findOrderByIdAction()
    {
        try {
            ...
            if ($order_id = $this->getRequest()->getParam('order_id')) {
                ...
                $order['is_return_request'] = (int) $order['is_return_request'];
                $order['order_status'] = $status[$order['order_status']];
                ...
                $order['tracking_type'] = (int) $order['tracking_type'];
                ...
                $return_within = (int) $settings['return_within'];
                $order['is_enable_return']  = false;
                $createdAt = $this->calculate_time_span($order['created_at']);
                $settings['enable_retrun'] = (bool) $settings['enable_retrun'];
        ...
        try {
            ...
            foreach ($items as $key => $value) {
                ...
                foreach ($varients as $vKey => $vValue) {
                    $vData = [
                        'format_id' => $order_id . '' . $value['product_id'],
